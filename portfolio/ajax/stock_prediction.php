...
for ($j = 0 ; $j < $num ; ++$j)
{
    ...
    if (!$t1) {
	$resulting = queryMysql("SELECT user FROM members ORDER BY user");
	$number = mysql_num_rows($result);
	$counter = 0;
	for($p = 0 ; $p < $num ; ++$p){
		$newUser = $row[0];
		$t5 = mysql_num_rows(queryMysql("SELECT * FROM friends WHERE user='$newUser' AND friend='$row[0]'"));
		if ($t5) $counter = $counter + 1;
	}
	
	echo " [<a href='members.php?add=".$row[0]    . "'>$counter of your friends are following this user. Would you like to follow as well?</a>]";
